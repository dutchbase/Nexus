#!/usr/bin/env node
// Loads tests/e2e/fixtures/seed.sql into DATABASE_URL via psql.
//
...
for (const v of REQUIRED_PATH_VARS) {
  if (!process.env[v]) {
    console.error(`seed.ts: ${v} is required (run tests/e2e/git-fixtures/create-fixtures.sh first and export its FIXTURE_REPO_* output)`);
    process.exit(1);
  }
}
