...
# SCRIPT_DIR is tests/e2e/git-fixtures, so ONE dirname lands at tests/e2e/.
# The fixture root must stay inside tests/e2e/ so tests/e2e/.gitignore covers
# it — one level higher and the generated repos show up as untracked
# embedded git repositories in `git status`.
...
if [[ -z "$FIXTURES_ROOT" ]]; then
    FIXTURES_ROOT="$E2E_DIR/.fixtures-tmp"
else
    # Make it absolute if relative
    if [[ ! "$FIXTURES_ROOT" = /* ]]; then
        FIXTURES_ROOT="$E2E_DIR/$FIXTURES_ROOT"
    fi
fi
