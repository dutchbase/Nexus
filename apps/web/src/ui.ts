      ...
      ${/^\/admin\/tickets\/[^/]+$/.test(path) ? `
        ...
        function renderSkills(){
          ...
          chips.replaceChildren(...selected.map(input=>{
            ...
            if(auto&&input.dataset.overridable===undefined){const tag=document.createElement("small");tag.textContent="auto";chip.append(tag)}
        ...
        async function persistSkills(){
          const skill_ids=toggles.filter(input=>input.checked&&input.dataset.auto===undefined).map(input=>input.value);
