...
export async function render(url: URL, _session: Session, _metrics: Record<string, number>): Promise<PageResult> {
  ...
  if (runPageMatch) {
    const run = (await pool.query(
      `SELECT ar.*,t.ticket_number,t.status ticket_status,p.name project_name,ps.content rendered_prompt,ps.phase prompt_name,ps.metadata_json->'promptVersionIds' prompt_versions,
