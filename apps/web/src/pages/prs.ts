...
function renderDetail(item: any, aiReviews: any[], conflictResolutions: any[]): PageResult {
  ...
  const body = `<div class="eyebrow">${escapeHtml(item.project_name)} · ${escapeHtml(item.repository)}</div>
    ...
    <dialog data-create-ticket-dialog aria-label="Create follow-up ticket"><div class="card-head">Create follow-up ticket</div><form data-create-ticket-form><div class="card-body"><label class="field"><span>Title</span><input name="title" required></label><label class="field"><span>Feedback</span><textarea name="feedback" rows="4" required></textarea></label><label class="field"><span>Description</span><textarea name="description" rows="4" required readonly></textarea></label><p class="error" role="alert"></p></div><div style="padding:12px 18px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:8px"><button class="button" type="button" data-close-dialog>Cancel</button><button class="button" type="button" data-generate-follow-up-description>Generate description</button><button class="button primary" type="submit" disabled>Create</button></div></form></dialog>
