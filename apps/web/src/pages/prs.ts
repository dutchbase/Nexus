...
export async function render(url: URL, _session: Session, _metrics: Record<string, number>): Promise<PageResult> {
  if (url.pathname === "/admin/pull-requests") {
    ...
    const rows = pullRequests.rows.map((item) => {
      ...
      return `<a class="ticket-row prs-row" href="${href}"><span class="mono">#${escapeHtml(item.number)}</span><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.project_name)}</span><span class="status ${stateBadge.cls}">${escapeHtml(stateBadge.label)}</span><span class="status ${aiBadge.cls}">${escapeHtml(aiBadge.label)}</span><span>${item.merge_conflicts ? `<span class="status danger">Conflicts</span>` : ""}</span><time>${item.created_at_provider ? escapeHtml(new Date(item.created_at_provider).toLocaleDateString("nl-NL")) : "—"}</time><div class="pr-actions"><a class="button" href="${href}" data-pr-approve>Approve &amp; Merge</a><a class="button" href="${href}" data-pr-ai-review-merge>AI Review &amp; Approve</a><a class="button" href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer">Open on GitHub ↗</a></div></a>`;
    }).join("");
