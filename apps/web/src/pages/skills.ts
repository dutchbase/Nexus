  ...
  if (skillPageMatch) {
    ...
    const body = `<div class="eyebrow">SKILL.md registry / Skills</div><h1 style="max-width:24ch">${escapeHtml(skill.name)}</h1>
      ...
      <div class="grid two"><section class="card"><div class="card-head">Reference</div><div class="card-body" style="display:flex;flex-direction:column;gap:16px">
        ...
        <div><label class="field"><span>Resolved path</span><div style="display:flex;gap:8px;align-items:center"><input type="text" name="filesystem_path" data-skill-path value="${escapeHtml(skill.filesystem_path || "")}" readonly style="flex:1;border:1px solid var(--border);background:var(--bg);border-radius:4px;padding:9px 11px;font-size:13px;cursor:not-allowed">${validation.valid ? `<span style="font-size:11.5px;font-weight:600;color:var(--t-ok);background:var(--s-ok);padding:3px 9px;border-radius:3px">Resolves</span>` : ""}</div></label></div>
