import { escapeHtml, fmtDate, keysetCondition, nextCursor, pageRequest, pagerHtml, pool, promptVersionsLabel, statusBadge } from "./shared.ts";
...
function filters(url: URL) {
  ...
  if (to) { values.push(`${to}T00:00:00.000Z`); conditions.push(`ar.started_at < $${values.length}::timestamptz + interval '1 day'`); }
  ...
  for (const [param, column] of [["run_type", "ar.run_type"], ["model", "ar.model"], ["usage_status", "ar.ai_usage_status"]] as const) {
...
export async function render(url: URL, _session: Session, _metrics: Record<string, number>): Promise<PageResult> {
  ...
  const rows = runs.map((run) => `<div class="ticket-row aiusage-row"><span data-label="Started"><a href="/admin/runs/${escapeHtml(run.id)}">${escapeHtml(run.started_at ? new Date(run.started_at).toLocaleString("nl-NL") : "Not started")}</a></span><strong data-label="Lifecycle">${escapeHtml(run.run_type ?? "—")}</strong><span data-label="Model / provider">${escapeHtml(run.model ?? "—")} · ${escapeHtml(run.provider ?? "—")}</span><span data-label="Tokens">${run.ai_usage_status === "captured" ? `${escapeHtml(run.input_tokens)} in · ${escapeHtml(run.output_tokens)} out · ${escapeHtml(run.total_tokens)} total` : "Not captured"}</span><span data-label="Cost">${escapeHtml(usd(run.estimated_cost_usd))}</span><span data-label="Ticket / PR">${run.ticket_number ? `<a href="/admin/tickets/${escapeHtml(encodeURIComponent(run.ticket_number))}">${escapeHtml(run.ticket_number)}</a>` : "—"}${run.pr_id ? ` · <a href="/admin/pull-requests/${escapeHtml(run.pr_id)}">PR #${escapeHtml(run.pr_number)}</a>` : ""}</span><span data-label="Prompt">${escapeHtml(run.prompt_name ?? "—")}${promptVersionsLabel(run.prompt_versions) ? ` · ${escapeHtml(promptVersionsLabel(run.prompt_versions))}` : ""}</span><span data-label="Status">${statusBadge(usageLabel(run.ai_usage_status))}</span></div>`).join("");
  ...
  const body = `<div class="eyebrow">Operate</div><h1>AI usage</h1>
    ...
    <form class="toolbar" data-ai-usage-filters style="margin-top:16px;display:flex;gap:8px;flex-wrap:wrap"><label>From <input name="from" placeholder="dd-mm-yyyy" value="${escapeHtml(fmtDate(from) === "—" ? "" : fmtDate(from))}"></label><label>To <input name="to" placeholder="dd-mm-yyyy" value="${escapeHtml(fmtDate(to) === "—" ? "" : fmtDate(to))}"></label><label>Project ${input("project", url.searchParams.get("project") ?? "")}</label><label>Lifecycle <select name="lifecycle"><option value="">All</option>${["planning", "execution", "pr_work"].map((value) => `<option value="${value}"${url.searchParams.get("lifecycle") === value ? " selected" : ""}>${value === "pr_work" ? "PR work" : value}</option>`).join("")}</select></label><label>Run type ${input("run_type", url.searchParams.get("run_type") ?? "")}</label><label>Model ${input("model", url.searchParams.get("model") ?? "")}</label><label>Usage status ${input("usage_status", url.searchParams.get("usage_status") ?? "")}</label><label>Ticket / PR ${input("search", url.searchParams.get("search") ?? "", "search")}</label><label><input type="checkbox" name="all_time" value="1"${allTime ? " checked" : ""}> All time</label><button class="button" type="submit">Filter</button><a class="button" href="/admin/ai-usage">Reset</a></form>
    <section class="card"><div class="list-head aiusage-head"><span>Started</span><span>Lifecycle</span><span>Model / provider</span><span>Tokens</span><span>Cost</span><span>Ticket / PR</span><span>Prompt</span><span>Status</span></div>${rows || `<div class="card-body">No AI invocations match these filters.</div>`}</section>${pagerHtml(url, nextCursor(runs, limit, "started_at"))}`;
