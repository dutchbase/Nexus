  ...
  if (formPageMatch) {
    ...
    const settingsPanel = `<section class="card"><div class="card-head">Form settings</div><div class="card-body">
      <form data-form-settings>
        ...
        <label class="field"><span>CAPTCHA</span><select name="captcha_mode"><option value="honeypot"${(settings.captcha_mode ?? "honeypot") === "honeypot" ? " selected" : ""}>Honeypot only</option><option value="honeypot_captcha"${settings.captcha_mode === "honeypot_captcha" ? " selected" : ""}>Honeypot + CAPTCHA</option></select></label>
