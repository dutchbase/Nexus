...
export async function render(url: URL, _session: Session, _metrics: Record<string, number>): Promise<PageResult> {
  if (url.pathname === "/admin/projects") {
    ...
    const body = `<div class="eyebrow">Configure / projects.yaml</div><h1>Projects</h1>
      ...
      <div class="grid">${projects.map((project) =>
  ...
  if (projectPageMatch) {
    ...
    const overviewPanel = `<section class="card"><div class="card-head">Paths & repository</div><div class="card-body"><form data-project-form data-project-id="${project.id}">
      ...
      ${["install", "lint", "typecheck", "test", "build"].map((cmd) =>
        `<input type="hidden" name="cmd_${cmd}" value="${escapeHtml(validationCommands[cmd] || "")}">`,
      ).join("")}</form></div></section>
      <section class="card"><div class="card-head">Validation commands</div><div class="card-body" style="display:flex;flex-direction:column;gap:8px">${["install", "lint", "typecheck", "test", "build"].map((cmd) =>
        `<div style="display:flex;gap:8px;align-items:center"><span class="mono" style="min-width:80px;color:var(--text3)">${escapeHtml(cmd)}</span><input form="[data-project-form]" data-cmd="${cmd}" style="flex:1" value="${escapeHtml(validationCommands[cmd] || "")}" placeholder="Command"></div>`,
      ).join("")}</div></section>`;
    ...
    const skillsPanel = `<section class="card"><div class="card-head">Automatically attached to every ticket</div><div class="card-body">${skillsResult.rows.length > 0 ? skillsResult.rows.map((skill) =>
      `<div style="padding:8px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border)"><div><strong>${escapeHtml(skill.name)}</strong> <span class="mono" style="font-size:11px;color:var(--text3)">${escapeHtml(skill.slug)}</span> <span style="font-size:11px;font-weight:600;color:var(--text3)">${escapeHtml(skill.category)}</span> <span style="font-size:11px;color:var(--text3)">v${skill.version}</span></div><label><input type="checkbox" ${skill.allow_ticket_override ? "checked" : ""} disabled> <small>Override</small></label></div>`,
    ).join("") : "<p>No automatic skills attached.</p>"}</div></section>`;
