...
const safeFields = (fields: any[]) => fields.filter((field) => !["static", "hidden"].includes(field.field_type) && !["project_id", "submitter_name", "submitter_email"].includes(field.field_key));
...
async function list(url: URL, session: Session) {
  ...
  const create = enabled.length ? `<details class="card"><summary class="card-head">New ticket</summary><form class="card-body" data-ticket-form data-create data-fields="${fieldData(fields)}"><label class="field"><span>Project</span><select name="project_id" required><option value="">Choose a project</option>${createOptions}</select></label>${formControls(fields, [], {}, "reporter", { uploadUrl: "/api/projects/{project_id}/uploads", disabled: true })}<button class="button primary" type="submit">Submit ticket</button><p class="error" role="alert"></p></form></details>` : "";
...
async function detail(ref: string, session: Session) {
  ...
  const jamLabels: Record<string, string> = { ready: "Details imported", partial: "Some details imported", failed: "Details could not be imported", not_configured: "Details could not be imported", queued: "Import pending", fetching: "Import pending" };
  const jamState = String(ticket.jam_import?.state ?? "queued");
  const jam = ticket.jam_url ? `<section class="card"><div class="card-head">Jam link</div><div class="card-body"><a href="${escapeHtml(ticket.jam_url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(ticket.jam_url)}</a><p data-jam-outcome${["queued", "fetching"].includes(jamState) ? ` data-jam-pending data-ticket-ref="${escapeHtml(ticket.ticket_number)}"` : ""}>${escapeHtml(jamLabels[jamState] ?? "Import pending")}</p></div></section>` : "";
  ...
  return { status: 200, title: ticket.ticket_number, body: `<p><a href="/tickets">← Tickets</a></p><div class="page-head"><div><span class="mono">${escapeHtml(ticket.ticket_number)}</span><h1>${escapeHtml(ticket.title)}</h1><p>${escapeHtml(ticket.project_name)} · submission updated ${date(ticket.submission_updated_at)}</p></div>${deletion}</div><section class="card"><div class="card-head">Original submission</div><div class="card-body"><dl>${details}</dl></div></section>${jam}<section class="card"><div class="card-head">Edit submission</div><form class="card-body" data-ticket-form data-ticket-id="${escapeHtml(ticket.id)}" data-project-id="${escapeHtml(ticket.project_id)}" data-fields="${fieldData(fields)}"><input type="hidden" name="submission_revision" value="${ticket.submission_revision}">${formControls(fields, [], values, "reporter", { uploadUrl: `/api/projects/${ticket.project_id}/uploads`, existing: attachments })}<button class="button primary" type="submit">Save changes</button><p class="error" role="alert"></p></form></section><section class="card"><div class="card-head">Images</div><div class="card-body">${images}</div></section>` };
}
