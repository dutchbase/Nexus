...
export function contentSecurityNonce() {
  return randomBytes(18).toString("base64url");
}

export function securityHeaders(production = process.env.NODE_ENV === "production", nonce = contentSecurityNonce()) {
  return {
    ...
    "content-security-policy": `default-src 'self'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'; object-src 'none'; script-src 'self' 'nonce-${nonce}'; style-src 'self' 'unsafe-inline'; style-src-elem 'self'; img-src 'self' data:; connect-src 'self'`,
