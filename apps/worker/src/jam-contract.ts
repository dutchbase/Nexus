...
import { bindJamToolArguments, boundedJamFetch, fetchJamContext } from "./jam-client.ts";
...
if (!token || !source) {
  ...
  const client = new Client({ name: "nexus-jam-contract", version: "1.0.0" }, { versionNegotiation: { mode: "legacy" } });
  try {
