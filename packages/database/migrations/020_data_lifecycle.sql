...
ALTER TABLE jobs ADD CONSTRAINT jobs_status_lifecycle_check CHECK (status IN ($s$queued$s$, $s$running$s$, $s$completed$s$, $s$published$s$, $s$failed$s$, $s$cancelled$s$, $s$blocked_auth$s$, $s$blocked_auth_configuration$s$));
...
ALTER TABLE execution_attempts ADD CONSTRAINT execution_attempts_status_lifecycle_check CHECK (validation_status IS NULL OR validation_status IN ($s$queued$s$, $s$pending$s$, $s$executing$s$, $s$validated$s$, $s$completed$s$, $s$failed$s$, $s$pr_creation_failed$s$, $s$cancelled$s$, $s$timed_out$s$));
