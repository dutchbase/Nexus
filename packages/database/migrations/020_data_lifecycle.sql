ALTER TABLE tickets ADD CONSTRAINT tickets_status_lifecycle_check CHECK (status IN ($s$Submitted$s$, $s$Triage$s$, $s$Needs Information$s$, $s$Rejected$s$, $s$Approved for Planning$s$, $s$Planning Queued$s$, $s$Planning$s$, $s$Planning Failed$s$, $s$Plan Ready for Review$s$, $s$Plan Approved$s$, $s$Plan Revision Requested$s$, $s$Plan Revision Queued$s$, $s$Execution Queued$s$, $s$Executing$s$, $s$Validating$s$, $s$Validation Failed$s$, $s$Execution Failed$s$, $s$PR Ready for Review$s$, $s$PR Approved$s$, $s$PR Changes Requested$s$, $s$PR Creation Failed$s$, $s$Merged$s$, $s$Completed$s$, $s$Closed Without Merge$s$, $s$Cancelled$s$, $s$Archived$s$));
ALTER TABLE jobs ADD CONSTRAINT jobs_status_lifecycle_check CHECK (status IN ($s$queued$s$, $s$running$s$, $s$completed$s$, $s$failed$s$, $s$cancelled$s$, $s$blocked_auth$s$, $s$blocked_auth_configuration$s$));
ALTER TABLE agent_runs ADD CONSTRAINT agent_runs_status_lifecycle_check CHECK (status IS NULL OR status IN ($s$queued$s$, $s$running$s$, $s$cancellation_requested$s$, $s$cancelled$s$, $s$timed_out$s$, $s$completed$s$, $s$failed$s$));
ALTER TABLE execution_attempts ADD CONSTRAINT execution_attempts_status_lifecycle_check CHECK (validation_status IS NULL OR validation_status IN ($s$queued$s$, $s$pending$s$, $s$executing$s$, $s$validated$s$, $s$completed$s$, $s$published$s$, $s$failed$s$, $s$pr_creation_failed$s$, $s$cancelled$s$, $s$timed_out$s$));
