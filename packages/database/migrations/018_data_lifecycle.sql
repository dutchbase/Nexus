ALTER TABLE tickets ADD CONSTRAINT tickets_status_lifecycle_check CHECK (status IN ($s$Submitted$s$, $s$Triage$s$, $s$Rejected$s$, $s$Approved for Planning$s$, $s$Planning Queued$s$, $s$Planning$s$, $s$Planning Failed$s$, $s$Plan Ready for Review$s$, $s$Plan Approved$s$, $s$Plan Revision Requested$s$, $s$Plan Revision Queued$s$, $s$Execution Queued$s$, $s$Executing$s$, $s$Validating$s$, $s$Validation Failed$s$, $s$Execution Failed$s$, $s$PR Ready for Review$s$, $s$PR Approved$s$, $s$PR Changes Requested$s$, $s$PR Creation Failed$s$, $s$Merged$s$, $s$Completed$s$, $s$Closed Without Merge$s$, $s$Cancelled$s$));
ALTER TABLE jobs ADD CONSTRAINT jobs_status_lifecycle_check CHECK (status IN ($s$queued$s$, $s$running$s$, $s$completed$s$, $s$failed$s$, $s$cancelled$s$));
