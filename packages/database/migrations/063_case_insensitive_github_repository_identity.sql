-- Installations that already recorded the original 062 still need the same
-- case-insensitive identity policy. The body is deliberately self-contained:
-- migrate() correctly skips applied SQL and cannot replay a historical file.
DO $$
DECLARE
...
BEGIN
  FOR duplicate_group IN
    ...
    HAVING count(*) > 1
  LOOP
    SELECT id INTO survivor_id FROM projects
    ...
    UPDATE projects
    SET enabled=false, github_owner=NULL, github_repository=NULL,
        config_json=config_json || jsonb_build_object(
          '_migration_063_repository_deduplication',
          jsonb_build_object('survivor_project_id',survivor_id,'github_owner',duplicate_group.owner_key,'github_repository',duplicate_group.repository_key)
        ), updated_at=now()
