...
describe("resolvedAiFor with a system-level override", () => {
  ...
  it("uses a system phase-specific override ahead of the system default", () => {
    const systemAi = { default: { model: "sonnet", reasoning_level: "high" }, planning: { model: "deepseek", reasoning_level: "low" }, execution: { model: null, reasoning_level: null }, repair: { model: null, reasoning_level: null } };
    expect(resolvedAiFor(ticketWithoutOverrides, project, "planning", systemAi))
      .toEqual({ model: "deepseek", reasoning_level: "low" });
  });
